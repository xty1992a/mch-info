...
function isBankCard(cardNumber) {
  ...
  var bankCardNum = cardNumber.replace(/\s+/g, "");
  var oddArr = [];
  var evenArr = [];
  for (var i = bankCardNum.length; i > 0; i--) {
    var str = bankCardNum.substr(bankCardNum.length - i, 1);
    if (i % 2 === 1) {
      oddArr.push(str);
    }
    else {
      var m = parseInt(str) * 2;
      if (m <= 9) {
        evenArr.push(m);
      }
      else {
  ...
  var sumOdd = eval(oddArr.join("+"));
  var sumEven = eval(evenArr.join("+"));
...
export default {
  "default": () => true,
  ID: v => idValidator.isValid(v),
  bankAccount: isBankCard
};
