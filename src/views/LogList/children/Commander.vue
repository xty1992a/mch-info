<template>
  <el-dropdown trigger="click" @command="commandHandler">
    ...
    <el-dropdown-menu slot="dropdown" class="log-set-dropdown">
      ...
      <el-dropdown-item command="deleteItem" v-role="[$roles.MERCHANT]"
        >删除
      </el-dropdown-item>
      <el-dropdown-item
        command="deleteItem"
      ...
      <el-dropdown-item
        command="repeatItem"
        v-role="[$roles.AGENT, $roles.SERVICE]"
        :disabled="data.channelId === 0"
        >复制进件
      </el-dropdown-item>
      <el-dropdown-item
        command="goToAliLh"
        v-role="[$roles.AGENT]"
        :disabled="!data.zfblhUrl && data.zfblhRecord === 0"
        >参加蓝海
      </el-dropdown-item>
      <el-dropdown-item
        command="goToWxLz"
        v-role="[$roles.AGENT]"
        :disabled="!data.wxlzUrl && data.wxlzRecord === 0"
        >参加绿洲
      </el-dropdown-item>
      <el-dropdown-item
        command="editCart"
        :disabled="data.auditStatus !== 2 && data.payeeType === 8"
        v-role="[$roles.AGENT, $roles.MERCHANT]"
        >修改结算卡
      </el-dropdown-item>
    </el-dropdown-menu>
