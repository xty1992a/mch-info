  ...
  <div class="chanel-item">
    ...
    >
      ...
      <el-col class="pay-cell pay-type" :span="10">
        <div class="pay-item" :class="map[it.text]===data.key?'active':''" v-for="it in data.payTypes" :key="it.text">
          <svg-icon :icon="it.type"/>
          <span>{{it.text}}</span>
        </div>
      </el-col>
      <el-col class="pay-cell pay-set" :span="2" style="text-align: center;">
        <el-button class="btn-text" @click="toggle" :disabled="!data.merchantParamList.length">配置</el-button>
      </el-col>
      ...
      <el-col class="pay-cell pay-index" :span="4">
        <span class="index-btn">
          <i :class="index === 0 ? 'disabled' : ''" class="el-icon-top" @click="changeIndex('up')"></i>
          <i :class="index === total - 1 ? 'disabled' : ''" class="el-icon-bottom" @click="changeIndex('down')"></i>
        </span>
        <span class="btn-text" v-show="index !== 0" @click="changeIndex('top')">置顶</span>
      </el-col>
    </el-row>
      ...
      <el-collapse v-model="showList">
        <el-collapse-item name="1">
          <div class="chanel-set-block">
            <el-form inline label-width="180px" label-position="right" v-model="formData">
              <el-form-item :prop="item.key" :label="item.name" v-for="item in data.merchantParamList" :key="item.key">
                <template v-if="item.key!=='refundCertificate'">
                  <el-input placeholder="请输入" v-model="formData[item.key]"></el-input>
                </template>
                <template v-else>
                  <FileBtn v-model="formData[item.key]"/>
                </template>
              </el-form-item>
              <img :src="img" alt="">
