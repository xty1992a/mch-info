<template>
  <div class="examine-block" :class="isMobile ? 'mobile' : ''">
        ...
        <template v-if="isMobile">
          ...
          <template v-if="channelOptions.length">
            <van-cell
              ...
              :arrow-direction="showChannel ? 'down' : ''"
            />
            <RadioBox
    ...
    <footer class="page-foot">
      ...
      <el-button v-if="isExamine" type="primary" @click="confirm"
        >进件</el-button
      >
    </footer>
  </div>
...
const diff = (a, b) =>
  a.find(it => !b.includes(it)) || b.find(it => !a.includes(it));
...
export default {
  ...
  computed: {
    ...
    channelList() {
      return (this.data.channelList || []).map(it => ({
        ...it,
