<template>
  <van-popup class="m-log-filter" v-model="show" position="right" @closed="close" style="height: 100%;width: 100%;">
    <div style="height: 300px;background-color: #fff;">
      <van-navbar title="筛选" left-text="返回" right-text="确定" @click-left="onCancel" @click-right="onConfirm"/>
      <section>
        <el-form v-model="formData" label-position="top">
          ...
          <el-form-item label="审核时间">
            <FDateRange :is-mobile="true" :data="{}" v-model="formData.requestTime"/>
          </el-form-item>
          <el-form-item label="审核时间">
            <FDateRange :is-mobile="true" :data="{}" v-model="formData.mchTime"/>
          </el-form-item>
...
<script>
  ...
  export default {
    name: "MFeeEditor",
    mixins: [Common],
    components: { VanPopup: Popup, VanNavbar: NavBar, RadioBox },
    data() {
      return {
        show: false
      };
