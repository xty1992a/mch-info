  ...
  >
    <section class="log-filter-body" @keypress.enter="onConfirm">
      ...
      >
        ...
        >
          <el-input
            v-model="formData.subMerchantId"
            placeholder="请输入商户号"
          />
        </el-form-item>
        <el-form-item
        ...
        <el-form-item
          ...
          style="display: block;width: 100%;"
        >
          <el-checkbox-group v-model="formData.businessLicenceType">
            <el-checkbox
              v-for="item in licenceTypeOptions"
              :key="item.value"
              :label="item.value"
            >
              {{ item.label }}
...
export default {
  ...
  computed: {
    auditStatus: {
      get() {
        return this.formData.auditStatus
          ? this.formData.auditStatus.split(",")
          : [];
      },
      set(v) {
