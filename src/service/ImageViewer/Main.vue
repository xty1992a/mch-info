<template>
  <van-popup class="image-viewer" :class="isMobile?'mobile':'desktop'" v-model="show" position="center" @closed="close">
    <section>
      <div class="nav-btn prev" @click="prev"><i class="el-icon-arrow-left"></i></div>
      <div class="nav-btn next" @click="next"><i class="el-icon-arrow-right"></i></div>
      <van-swipe :autoplay="0"
                 ref="swipe"
                 indicator-color="white"
                 style="width: 100%;height: 100%;"
                 :show-indicators="false">
        <van-swipe-item v-for="(img, i) in images" :key="i" @click="onTouchEnd" @mousewheel.native="mouseWheel">
...
<script>
  ...
  export default {
    name: "ImageViewer",
    components: { VanPopup: Popup, VanSwipe: Swipe, VanSwipeItem: SwipeItem, VanImage: Image },
    data() {
      return {
        show: false,
        current: 0,
