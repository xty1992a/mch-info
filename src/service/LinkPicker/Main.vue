<template>
  ...
  >
    <div
      class="link-picker-body"
      :class="isMobile ? 'mobile' : 'desktop'"
      v-loading="onInit"
    >
      <header>
        {{ title }} <i class="el-icon-error" @click="show = false"></i>
      </header>
      <section>
        <div class="search">
          <el-input
        ...
        <div class="picker">
          <div
            class="loading"
            v-if="loading"
            style="padding: 0 10px;line-height: 40px;"
          >
            加载中...
          </div>
...
export default {
  ...
  methods: {
    ...
    onConfirm() {
      this.resolve({
        success: true,
        value: this.pickedItems.map(it => it.value).join(","),
        data: this.pickedItems
      });
      this.show = false;
    },
  ...
  computed: {
    ...
    matchList() {
      ...
      return this.keywords
        ? labels.filter(it => it.includes(this.keywords))
        : labels;
    }
  },
  beforeDestroy() {
